...
var _parser = createCommonjsModule(function (module, exports) {
 ...
 * @returns {*}
 */
function buildOperationStringHelper(root, table, node, variables, params, left_side, aliases, qb) {
    ...
    if (node.type === Nodes.OPERATION) {
        ...
        const a_bos = buildOperationStringHelper(root, table, a, variables, params, true, aliases, qb);

        const b_bos = buildOperationStringHelper(root, null, b, variables, params, false, aliases, qb);
        ...
        // Locate the query
        const target_query = root.filter(x => x.type === 'QUERY' && x.name === node.name)[0];

        if (typeof target_query === 'undefined') throw new Error(`Could not find query '${node.name}`);
        ...
        return {
            text: '?',
            variables: [QueryProcessor$1(qb.flavour).process(root, target_query, {
                variables: vmap
            })]
        };
    } else if (left_side) {
        return {
            text: table !== null ? typeof aliases[node.value] !== 'undefined' ? node.alias : `${table}.${node.value}` : node.value,
            variables: []
        };
    } else {
        ...
        return {
            text: '?',
            variables: [table !== null ? typeof aliases[node.value] !== 'undefined' ? node.alias : `${table}.${node.value}` : node.value]
        };
    }
}
...
class Helpers {
    ...
    static buildOperationString(root, table, node, variables, aliases, qb) {
        ...
        return buildOperationStringHelper(root, table, node, variables, [], false, aliases || {}, qb);
    }
...
class JoinProcessor extends Processor {
    ...
    _processJoin(root, node, variables, aliases) {
        ...
        const selectors = on.map(x => {
            const op = Helpers.buildOperationString(root, node.table, x, variables, aliases, this._qb);
    ...
    process(root, node, variables, qb) {
        const aliases = [];
        const joins = node.nodes.filter(x => x.type === Nodes.JOIN).map(x => this._processJoin(root, x, variables, aliases));
...
class QueryProcessor extends Processor {
    ...
    _processTable(root, node, variables, options) {
        ...
        qb = JoinProcessor$1(this._qb).process(root, node, variables, qb);
        ...
        // Add grouping
        if (typeof groupBy !== 'undefined' && groupBy !== null) qb.group(groupBy);
...
const dql = flavor => function (strings) {
    ...
    const trees = _parser.parse(result);
